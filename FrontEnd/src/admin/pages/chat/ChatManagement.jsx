...
const ChatManagement = () => {
  ...
  const subscribeToAdminTopics = () => {
    ...
    stompClientRef.current.subscribe('/topic/admin/chat/messages', (message) => {
      ...
      if (selectedConversation && messageData.conversationId === selectedConversation.id) {
        // Thêm tất cả tin nhắn vào danh sách để admin có thể thấy
        // Chỉ kiểm tra để tránh duplicate tin nhắn từ chính admin hiện tại
        const currentAdminId = localStorage.getItem('adminId');
        if (!messageData.isFromAdmin || messageData.senderId != currentAdminId) {
          setMessages(prev => {
  ...
  const handleSelectConversation = (conversation) => {
    ...
    // Re-subscribe to admin topics to include the new conversation
