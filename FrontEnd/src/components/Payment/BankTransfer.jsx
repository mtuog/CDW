...
const BankTransfer = () => {
    ...
    useEffect(() => {
        const fetchOrderDetails = async () => {
            try {
                ...
                const response = await axios.get(`http://${BACKEND_URL_HTTP}/api/orders/${orderId}`, {
        ...
        const fetchBankAccounts = async () => {
            try {
                const response = await axios.get(`http://${BACKEND_URL_HTTP}/api/bank-payments/accounts`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post(`http://${BACKEND_URL_HTTP}/api/bank-payments/orders/${orderId}`, 
                paymentData, 
                { headers: { Authorization: `Bearer ${token}` } }
            );
            ...
            const errorMsg = err.response?.data?.error || 'Đã xảy ra lỗi khi gửi thông tin thanh toán. Vui lòng thử lại.';
            setError(errorMsg);
    ...
    const generateDynamicVietQR = async (bankAccount, order) => {
        ...
        try {
            const response = await axios.post(
                `http://${BACKEND_URL_HTTP}/api/bank-payments/generate-qr`,
                {
